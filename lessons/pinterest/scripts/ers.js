...
function goToPage() {
	...
	var ajaxTar = 'pinterest_lessons/lesson' + lessonNumber + '/pinterest_lesson_' + currentPage + ".html";
...
function buildNav() {
	...
	for ( var ind in pages.slides ) {
		...
		if (parseInt(ind) == parseInt(currentPage)) {
			var li = '<li class="active_thumb" style="background-image: url(\'pinterest_lessons/lesson' + lessonNumber + '/pinterest' + ind + '.png\');"><span>' + ind + '</span></li>';
			console.log("list item in if: ",li);
		} else {
			var li = '<li style="background-image: url(\'pinterest_lessons/lesson' + lessonNumber + '/pinterest' + ind + '.png\');"><span>' + ind + '</span></li>';
		}
